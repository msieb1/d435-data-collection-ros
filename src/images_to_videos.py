...
parser.add_argument(
    ...
    help='Output path to debug video.')
parser.add_argument(
    '--fps', type=str, default='',
    help='fps of input streams.')

def create_vids(view_dirs, vid_paths, debug_path=None,
                debug_lhs_view=0, debug_rhs_view=1, fps=29):
  ...
  for (view_dir, vidpath) in zip(view_dirs, vid_paths):
    ...
    -oac copy -o %s' % (view_dir, fps, vidpath)
    # encode_vid_cmd = r'ffmpeg -framerate %s -i %s/*.png %s' % (fps, view_dir, vidpath) 
...
def main():
  ...
  create_vids(view_dirs, vid_paths, FLAGS.debug_path,
              FLAGS.debug_lhs_view, FLAGS.debug_rhs_view, FLAGS.fps)
